...
export function ProgressiveOnboarding() {
  ...
  return (
    <>
      {guide}
      <button type="button" onClick={async () => { await updateAsync({ action: "restart-guide" }); router.push("/dashboard"); }} className="fixed bottom-5 left-20 z-30 inline-flex size-9 items-center justify-center rounded-full border border-border bg-surface text-text-muted shadow-card-soft transition hover:bg-surface-secondary hover:text-text-primary" title="Restart getting started guide" aria-label="Restart getting started guide"><CircleHelp className="size-4" /></button>
    </>
  );
}
